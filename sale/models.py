...
class Item(models.Model):
    ...
    item_price = models.DecimalField(
        ...
        validators=[
            MinValueValidator(
                limit_value=0, message="You cannot pay to sell your product"
            )
        ],
    )
...
class Review(models.Model):
    ...
    owner = models.ForeignKey(
        CustomerUser, on_delete=models.CASCADE, related_name="customer_reviews"
    )
    audit = models.CharField(max_length=511)
    rating = models.PositiveIntegerField(
        validators=[
            MinValueValidator(limit_value=1, message="The Mimimum rating is 1"),
